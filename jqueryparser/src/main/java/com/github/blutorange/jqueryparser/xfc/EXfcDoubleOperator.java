...
public enum EXfcDoubleOperator implements IOperator<FilterCriterion, XfcContext, String>, IOperatorNameProviding {
	...
	private EXfcDoubleOperator(final int exactly, final Mapper mapper) {
		this(exactly, exactly, mapper);
	}

	private EXfcDoubleOperator(final int min, final int max, final Mapper mapper) {
		this.min = min;
		this.max = max;
		this.mapper = mapper;
	}
	...
	private interface Mapper {
		FilterCriterion map(String field, Double[] values) throws QueryBuilderEvaluatorException;
	}
}
