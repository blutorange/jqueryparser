...
enum EHibernateDoubleOperator
	...
	private EHibernateDoubleOperator(final int exactly, final Mapper mapper) {
		this(exactly, exactly, mapper);
	}

	private EHibernateDoubleOperator(final int min, final int max, final Mapper mapper) {
		this.min = min;
		this.max = max;
		this.mapper = mapper;
	}
	...
	@Override
	public Criterion operate(final HibernateContext context, final String field, @NonNull final String[] values) throws QueryBuilderEvaluatorException {
		if (values.length < min || values.length > max)
			throw new QueryBuilderEvaluatorException(Codes.ILLEGAL_NUMBER_OF_VALUES, String.valueOf(values.length));
		final Double[] intValues = new Double[values.length];
	...
	private interface Mapper {
		Criterion map(String field, Double[] values) throws QueryBuilderEvaluatorException;
	}
}
