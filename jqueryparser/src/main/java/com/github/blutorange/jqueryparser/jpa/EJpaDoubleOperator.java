...
enum EJpaDoubleOperator implements IOperator<Predicate, IJpaContext, IJpaRuleContext>, IOperatorNameProviding {
	...
	private EJpaDoubleOperator(final int exactly, final Mapper mapper) {
		this(exactly, exactly, mapper);
	}

	private EJpaDoubleOperator(final int min, final int max, final Mapper mapper) {
		this.min = min;
		this.max = max;
		this.mapper = mapper;
	}
	...
	public Predicate operate(final IJpaContext context, final IJpaRuleContext path, @NonNull final String[] values) throws QueryBuilderEvaluatorException {
		...
		return mapper.map(path.getCriteriaBuilder(), path.getDoublePath(), doubleValues);
	}
	...
	private interface Mapper {
		Predicate map(CriteriaBuilder criteriaBuilder, Path<? extends @Nullable Double> field, @NonNull Double[] values)
